...
public class CardIndexTest {
    ...
    public void numberOfCardIndices() {
        Assert.assertEquals("Number of distinct card indices", 13,
            CardIndex.values().length);
    }

    @Test
    public void valueOf() {
        // so that EclEmma reports full instruction coverage of CardIndex
        Assert.assertEquals("Value of \"" + JACK_STRING + "\"",
            CardIndex.valueOf(JACK_STRING), CardIndex.JACK);
    }
}